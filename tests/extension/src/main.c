...
static void test_01(void)
{
    ...
    while (1) {
        if (!k_msgq_get(&__zb_ext_msgq, &idx, K_FOREVER)) {
            ...
            __zb_chan_read(meta, (uint8_t *) &msg_data, meta->message_size, K_MSEC(500));
            switch (idx) {
            case zb_index_sensor_data: {
                ++count;
            ...
            case zb_index_start_measurement: {
            ...
            case zb_index_finish: {
                ...
                __zb_chan_pub(__zb_metadata_get_by_id(zb_index_start_measurement),
            ...
            memset(&msg_data, 0, sizeof(zb_channel_variant_t));
        }
    }
}
