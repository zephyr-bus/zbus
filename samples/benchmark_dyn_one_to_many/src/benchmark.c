...
#define S_TASK(name)                                                                    \
    ...
    {                                                                                   \
        ...
        while (!k_msgq_get(name.queue, &idx, K_FOREVER)) {                              \
            zbus_chan_claim(ZBUS_CHANNEL_GET(bm_channel),                      \
            ...
            zbus_chan_finish(ZBUS_CHANNEL_GET(bm_channel), K_NO_WAIT);         \
...
{
    struct external_data_msg *actual_message_data = NULL;
    zbus_chan_claim(ZBUS_CHANNEL_GET(bm_channel), (void *) &actual_message_data,
                    K_NO_WAIT);
...
void producer_thread(void)
{
    ...
    for (uint64_t internal_count = BYTES_TO_BE_SENT / BM_ONE_TO; internal_count > 0;
        ...
        zbus_chan_finish(ZBUS_CHANNEL_GET(bm_channel), K_NO_WAIT);
        zbus_chan_notify(ZBUS_CHANNEL_GET(bm_channel), K_MSEC(200));
    }
