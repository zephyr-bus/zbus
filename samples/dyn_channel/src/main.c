/*
 * Copyright (c) 2022 Citrinio
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include <stdint.h>
#include "kernel.h"
#include "zbus.h"

#include <logging/log.h>
#include "zbus.h"
LOG_MODULE_DECLARE(zbus, CONFIG_ZBUS_LOG_LEVEL);

/*! benchmark description
 * data variation from 1 to 256 bytes (1,2,4,8,16,32,64,128,256)
 * callback based first
 */

struct pkt {
    struct {
        uint8_t filter : 1;
        uint8_t body_size : 4;

    } header;
    uint8_t body[];
};

void filter_cb(zbus_channel_index_t idx);
ZBUS_SUBSCRIBER_REGISTER_CALLBACK(filter, filter_cb);

zbus_message_variant_t msg_received = {0};
void filter_cb(zbus_channel_index_t idx)
{
    struct pkt *filtered_data = NULL;
    ZBUS_ASSERT(idx == zbus_index_pkt_channel);
    zbus_dyn_chan_borrow(ZBUS_CHANNEL_METADATA_GET(pkt_channel), (void **) &filtered_data,
                         K_NO_WAIT);
    if (filtered_data->header.filter) {
        memset(filtered_data->body, 0, filtered_data->header.body_size);
    }
    zbus_dyn_chan_give_back(ZBUS_CHANNEL_METADATA_GET(pkt_channel), K_NO_WAIT);

    struct ack_msg dr = {1};
    zbus_chan_pub(data_ready, dr, K_NO_WAIT);
}

void main(void)
{
    struct version_msg v = {0};
    zbus_chan_read(version, v, K_NO_WAIT);

    printk("\n -> Sample dynamic filter version %u.%u-%u\n\n", v.major, v.minor, v.build);
}

ZBUS_SUBSCRIBER_REGISTER(producer, 4);
void producer_thread(void)
{
    struct pkt *msg          = NULL;
    zbus_channel_index_t idx = ZBUS_CHANNEL_COUNT;

    int i = 0;
    do {
        msg = k_malloc(i + 1);
        ZBUS_ASSERT(msg != NULL);
        memset(&msg->body, i, i);
        msg->header.body_size = i;
        msg->header.filter    = i % 2;
        zbus_dyn_chan_alloc(ZBUS_CHANNEL_METADATA_GET(pkt_channel), msg, i + 1,
                            K_NO_WAIT);
        zbus_dyn_chan_notify(ZBUS_CHANNEL_METADATA_GET(pkt_channel), K_NO_WAIT);
        ++i;
    } while ((i < 16) && !k_msgq_get(producer.queue, &idx, K_FOREVER));
}
K_THREAD_DEFINE(producer_thread_id, 1024, producer_thread, NULL, NULL, NULL, 5, 0, 5000);


ZBUS_SUBSCRIBER_REGISTER(consumer, 4);
void consumer_thread(void)
{
    struct pkt *received     = NULL;
    zbus_channel_index_t idx = ZBUS_CHANNEL_COUNT;
    while (!k_msgq_get(consumer.queue, &idx, K_FOREVER)) {
        ZBUS_ASSERT(idx == zbus_index_data_ready);
        zbus_dyn_chan_borrow(ZBUS_CHANNEL_METADATA_GET(pkt_channel), (void **) &received,
                             K_NO_WAIT);
        printk("Header(filter=%d,body_size=%02d)+Body(", received->header.filter,
               received->header.body_size);
        for (int i = 0; i < received->header.body_size; ++i) {
            printk("%02X", received->body[i]);
        }
        printk(")\n");
        zbus_dyn_chan_give_back(ZBUS_CHANNEL_METADATA_GET(pkt_channel), K_NO_WAIT);
        void *mem_to_be_free = NULL;
        zbus_dyn_chan_dealloc(ZBUS_CHANNEL_METADATA_GET(pkt_channel), &mem_to_be_free,
                              K_MSEC(500));
        k_free(mem_to_be_free);
        struct ack_msg a = {1};
        zbus_chan_pub(ack, a, K_MSEC(250));
    }
}

K_THREAD_DEFINE(consumer_thread_id, 1024, consumer_thread, NULL, NULL, NULL, 4, 0, 5000);
