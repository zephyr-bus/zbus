...
void fh1_cb(zb_channel_index_t idx);
ZB_SUBSCRIBER_REGISTER_CALLBACK(fast_handler1, fh1_cb);

void fh2_cb(zb_channel_index_t idx);
ZB_SUBSCRIBER_REGISTER_CALLBACK(fast_handler2, fh2_cb);

void fh3_cb(zb_channel_index_t idx);
ZB_SUBSCRIBER_REGISTER_CALLBACK(fast_handler3, fh3_cb);

void dh1_cb(zb_channel_index_t idx);
ZB_SUBSCRIBER_REGISTER_CALLBACK(delay_handler1, dh1_cb);

void dh2_cb(zb_channel_index_t idx);
ZB_SUBSCRIBER_REGISTER_CALLBACK(delay_handler2, dh2_cb);

void dh3_cb(zb_channel_index_t idx);
ZB_SUBSCRIBER_REGISTER_CALLBACK(delay_handler3, dh3_cb);

struct sensor_msg msg = {0};
void fh1_cb(zb_channel_index_t idx)
{
    zb_chan_read_by_index_unsafe(idx, msg, K_NO_WAIT);
...
void wq_dh_cb(struct k_work *item)
{
    ...
    zb_chan_read_by_index_unsafe(sens->idx, msg, K_NO_WAIT);
...
ZB_SUBSCRIBER_REGISTER(thread_handler1, 4);
void thread_handler1_task()
{
    zb_channel_index_t idx = ZB_CHANNEL_COUNT;
...
ZB_SUBSCRIBER_REGISTER(thread_handler2, 4);
...
ZB_SUBSCRIBER_REGISTER(thread_handler3, 4);
